...
namespace Client.MVVM.View
{
    ...
    {
        private static readonly List<DockingWindow> dockingWindows = new();

        //We use this collection to keep track of what windows we have open
        protected List<DockingWindow> OpenWindows = dockingWindows;
        ...
        protected bool TryDragTabToWindow(Point position, TabBase draggedTab)
        {
            ...
            */
            ...
            if (win == null)//If not, create a new one
            {
                ...
                win.SetWindowTitle($"LeagueBroadcast | {draggedTab.TabName}");
                win.Show();
            }
            else
            {
        ...
        private void Win_Closed(object? sender, EventArgs e)
        {
            if (sender is null)
                return;
            _ = OpenWindows.Remove(sender as DockingWindow);
        }
